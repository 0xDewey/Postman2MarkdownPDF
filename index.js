            ...
            if (item.item) {
              ...
              if (item.description) {
                ...
                item.item.forEach((subItem, subIndex) => {
                  markdownContent += `### Request : ${subItem.name}\n\n`;
                  ...
                  if (subItem.request.body && subItem.request.body.raw) {
                  ...
                  if (subItem.response && subItem.response.length > 0) {
...
rl.question("Please enter the path of the JSON file: ", (jsonFilePath) => {
  ...
  rl.question(
    "Please enter the path for the output Markdown file: ",
    (markdownFilePath) => {
      ...
      rl.question(
        "Please enter the path for the output PDF file: ",
        async (pdfFilePath) => {
          try {
            ...
            console.log(
              "Conversion from Markdown to PDF completed successfully."
            );
          } catch (error) {
            console.error(
              "Error during conversion from Markdown to PDF:",
              error.message
            );
          } finally {
            rl.close();
